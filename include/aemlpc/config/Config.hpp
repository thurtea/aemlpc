...
struct ListenPort {
    int slot = 1;
    ListenKind kind = ListenKind::Telnet;
    int port = 0;
    bool tls = false;
};
...
public:
    ...
    // Port table for Server::listen(). If the config has one or more
    // `listen:` lines (`listen: telnet 1122`, `listen: websocket 8080`,
    // `listen: telnet 8443 tls`), those lines are the table. Otherwise
    // a single telnet entry is synthesized from `port:`.
    const std::vector<ListenPort>& listenPorts() const { return listenPorts_; }
...
private:
    ...
    std::vector<ListenPort> listenPorts_;
    ...
    void addListenLine(const std::string& val);
